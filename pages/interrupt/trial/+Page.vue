<template>
  <div class="bg-white flex flex-col items-center justify-center h-full">
    <div class="p-4 lg:p-8 mx-auto !-mt-12 max-w-7xl">
      <div class="mx-auto grid max-w-2xl grid-cols-1 gap-x-8 gap-y-16 sm:gap-y-20 lg:mx-0 lg:max-w-none lg:grid-cols-2">
        <div class="lg:pr-8 lg:pt-4 flex flex-col items-center lg:items-start">
          <div class="lg:max-w-lg flex flex-col items-center lg:items-start">
            ...
            <p
              class="text-center lg:text-left mt-2 text-pretty text-xl lg:text-3xl font-semibold tracking-tight text-gray-900 sm:text-5xl">
              {{
                $t('subscriptions.start-trial-header', {
                  application: props.application?.name || 'Application'
                }) }}
            </p>
            <p class="text-center lg:text-left mt-6 text-sm lg:text-base/8 text-gray-900">
              {{
                $t('subscriptions.start-trial-desc', {
                  application: props.application?.name || 'Application'
                }) }}
            </p>
          </div>
          ...
          <div class="mt-6 flex flex-col items-start justify-center gap-x-6" v-else-if="!loading && !didStartTrial">
            <ThemedButton variant="default" class="w-56" icon="IconPlayerPlayFilled" @click="startTrial">
              {{ $t('subscriptions.start-trial-action') }}</ThemedButton>
            <a :href="`https://commons.libretexts.org/store/product/${props.application?.stripe_id}?mtm_source=libreone`"
              class="text-sm/6 font-semibold text-slate-500 mt-4 ml-0.5 text-center w-full lg:text-left hover:underline">
