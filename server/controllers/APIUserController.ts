...
export class APIUserController {
   ...
   */
  static parseAPIUserPermissions(config: APIUserPermissionConfig): APIUserPermission[] {
   ...
   */
  static mapAPIUserPermissionsToConfig(permissions: APIUserPermission[]): CreationAttributes<APIUserPermissionConfig> {
   ...
   */
  static async verifyAPIUserAuth(username: string, password: string, ip_address: string): Promise<APIUserAuthCheckOutput> {
    try {
      ...
      if (foundUser) {
        ...
        if (authorized) {
          ...
          return {
            isAuthorized: true,
            permissions: permissions ? this.parseAPIUserPermissions(permissions) : [],
          };
        }
      }
    } catch (e) {
   ...
   */
  public async createAPIUser(req: Request, res: Response) {
    ...
    await APIUserPermissionConfig.create({
      ...
      ...APIUserController.mapAPIUserPermissionsToConfig(props.permissions),
    });
   ...
   * @returns The fulfilled API response.
   */
  public async getAPIUser(req: Request, res: Response) {
    ...
    return res.send({
      data: {
        ...foundUser.get(),
        permissions: permissions ? APIUserController.parseAPIUserPermissions(permissions) : [],
      },
    });
  }
   ...
   * @returns The fulfilled API response.
   */
  public async getAllAPIUsers(req: Request, res: Response): Promise<Response> {
   ...
   * @returns The fulfilled API response.
   */
  public async updateAPIUser(req: Request, res: Response): Promise<Response> {
    const { id } = req.params as APIUserIDParams;
    ...
    if (permissions && Array.isArray(props.permissions)) {
      const configFields = APIUserController.mapAPIUserPermissionsToConfig([...API_USERS_PERMISSIONS]);
      permissions.update({
   ...
   * @returns The fulfilled API response.
   */
  public async deleteAPIUser(req: Request, res: Response): Promise<Response> {
    const { id } = req.params as APIUserIDParams;
