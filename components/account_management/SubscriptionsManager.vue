<template>
        ...
        <div>
          ...
          <p class="text-gray-600">
            <i18n-t keypath="subscriptions.description">
                <template #always>
                    <span class="italic font-bold">{{ $t('subscriptions.always') }}</span>
                </template>
            </i18n-t>
          </p>
        </div>
        <ThemedButton variant="default" @click="redeemCode" class="px-4 py-2 h-fit w-auto whitespace-nowrap mt-4 md:mt-0 md:ml-8">
          {{ $t('subscriptions.redeem-code') }}
        </ThemedButton>
      </div> 
  
      <div v-if="userLicenses.length === 0" class="text-center py-8 text-gray-500">
        {{ $t('subscriptions.no-active-subscriptions') }}
      </div>
      
      <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mb-8 max-h-[600px] overflow-y-auto">
        <LicenseCard
  ...
  <script lang="ts" setup>
    ...
    async function fetchUserLicenses() {
        ...
        const response = await axios.get(`/app-licenses/user/${pageContext.user.uuid}`);
    ...
    function browseLicenses() {
        window.location.href = 'https://commons.libretexts.org/bookstore';
    }
